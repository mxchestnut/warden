...
export function MatrixOnboardingModal({ onClose }: MatrixOnboardingModalProps) {
  ...
  const token = localStorage.getItem('access_token');
      if (token) {
  await fetch(`${import.meta.env.VITE_API_URL || 'https://warden.my'}/api/v1/auth/mark-matrix-onboarding-seen`, {
          method: 'POST',
          headers: {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4" role="dialog" aria-modal="true" aria-labelledby="matrix-onboarding-title">
      <div ref={dialogRef} className="bg-white rounded-lg p-8 max-w-2xl w-full shadow-xl max-h-[90vh] overflow-y-auto" role="document">
        <h2 id="matrix-onboarding-title" className="text-2xl font-bold text-gray-900 mb-6">Welcome to Warden Chat!</h2>
        ...
        <div className="space-y-4 text-gray-700">
          ...
          <div className="bg-blue-50 border-l-4 border-blue-500 p-4 rounded">
            ...
            <p>
              Matrix is an open-source, decentralized messaging protocol that powers Warden's real-time chat. 
          ...
          <div className="bg-yellow-50 border-l-4 border-yellow-500 p-4 rounded">
            ...
            <p>
              A Matrix account has been created for you automatically. To sign in to Element, 
              you'll need to set a password. You can do this from your <strong>Profile → Messaging</strong> section.
            </p>
          </div>
